...
// Cache clients per network to avoid creating new connections
...
server.setRequestHandler(ListToolsRequestSchema, async () => ({
  tools: [
    ...
    },
    {
      ...
      description: "Get transaction history for an address (sent or received)",
      inputSchema: {
        type: "object",
        properties: {
          ...
          filter: { 
            type: "string", 
            enum: ["from", "to", "both"],
            description: "Filter by sender (from), recipient (to), or both (default: both)" 
          },
          limit: { type: "number", description: "Max results (default: 50)" },
  ...
  try {
    switch (name) {
      ...
      case "get_coins": {
        ...
        return {
          content: [{ type: "text", text: JSON.stringify(coins, null, 2) }],
        };
      }
      ...
      case "get_latest_checkpoint": {
        ...
        return {
          content: [{ type: "text", text: JSON.stringify({ latestCheckpoint: checkpoint }, null, 2) }],
        };
      }
      ...
      case "get_chain_id": {
        ...
        return {
          content: [{ type: "text", text: JSON.stringify({ chainId }, null, 2) }],
        };
      }
      ...
      case "get_reference_gas_price": {
        ...
        return {
          content: [{ type: "text", text: JSON.stringify({ referenceGasPrice: gasPrice.toString() }, null, 2) }],
        };
      }
      ...
      case "get_stakes": {
        ...
        const stakes = await client.getStakes({
          owner: args?.address as string,
        });
        return {
          content: [{ type: "text", text: JSON.stringify(stakes, null, 2) }],
        };
      }
      ...
      case "dry_run_transaction": {
        ...
        const result = await client.dryRunTransactionBlock({
          transactionBlock: args?.txBytes as string,
        });
        return {
      ...
      case "request_faucet": {
        ...
        if (!faucetUrl) {
          throw new Error(`Faucet not available for ${net}`);
        }
      ...
      case "get_transaction_history": {
        ...
        if (filter === "from") {
          queryFilter = { FromAddress: address };
        } else if (filter === "to") {
          queryFilter = { ToAddress: address };
        } else {
          queryFilter = { FromOrToAddress: { addr: address } };
        }
...
async function main() {
  ...
  console.error("OneChain MCP server running on stdio");
}
